    ...
    @endisset
        <div class="row mx-auto justify-content-center">
            <div class="row py-5 mt-dk-80 mb-dk-80 px-3 pt-5 pb-10">
                ...
                <checkout-trajectories-info
                    :course='@json($course)'
                    :plan-selected-id='@json($pack_id)'
                    :price_subscription='@json($price_subscription)'
                    :price_enrollment='@json($price_enrollment)'
                ...
                <div class="col-12 col-lg-8 order-lg-1">
                    ...
                    <div class="needs-validation validation"
                        ...
                        @endisset
                        @isset($price_subscription->prices_id_stripe)
                            <input type="hidden" name="price_id" value="{{$price_subscription->prices_id_stripe}}" id="price_subscription_id">
                        @else
                            <input type="hidden" name="price_id" value="" id="price_subscription_id">
                        @endisset
                        <input type="hidden" name="price_id" value="{{optional($price_enrollment)->prices_id_stripe}}" id="price_enrollment_id">
                        <input type="hidden" name="course_id" value="{{$course->id}}" id="course_id">
                        <coupon-user :course='@json($course)'></coupon-user>
                        <checkout-promotion id="promoInpt" @isset($promotion):promotion='@json($promotion)' @endisset :course='@json($course)' :price_subscription='@json($price_subscription)' :promotions='@json($course->promotions()->IsPurchasable(true)->ActivePromotions(true)->get())'></checkout-promotion>

                        @empty($promotion)

                            <v-alert dense type="info" class="mb-10">
                                <template v-slot:prepend>
                                    <span class="v-icon notranslate v-alert__icon">@php echo Mdi::mdi('information-outline'); @endphp</span>
                                </template>
                                <span class="text-body-2">Recuerda que debes seleccionar primero la fecha del curso.</span>
                        ...
                        <course-plans-section class="mb-10"></course-plans-section>
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 col-sm d-flex align-items-center">
                                <img  src="/assets/images/logo/pagos_anuales.png" alt="" width="230px" class="ml-sm-auto">
                            </div>
                        </div>
                        ...
                        <div id="card-element"><!--Stripe.js injects the Card Element-->
                            <div class="d-lg-flex">
                                <div class="col-12 col-md-6 mr-2 pl-0">
...
@push('scripts')
    <script async src="{{ mix('/dist/js/trajectories.js') }}" defer></script>
