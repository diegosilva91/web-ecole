...
export default {
    props: ['type_course','optionsRequestSelected'],
    components: {
        InfiniteLoading,
    },
    data() {
    ...
    mounted() {
        if (this.optionsRequestSelected !== undefined && this.optionsRequestSelected !==null) {
            ...
            this.queryCourseType = 'type_course='+this.type_course;
            this.applyFilters();
            this.UpdateCourses(this.query);
        }
    },
    methods: {
        loadRequestFilters() {
            if(this.optionsRequestSelected !== undefined) {
                let indexArea;let slugArea;let indexCategories;let slugCategories;
                let index;let slug;
        ...
        applyFilters() {
            ...
            if (this.queryCourseType !== '') {
                query += 'type_course='+this.type_course;
            }
        ...
        async initQueries(query) {
            ...
            let data = await getCourses('courses/search', query, page, this.type_course, );
            console.log(data);
