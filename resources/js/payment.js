...
Vue.component('checkout-info', () => import(/* webpackChunkName: "dist/js/checkout-info" */  './components/Payment/CheckoutInfo.vue'));
Vue.component('course-plans', () => import(/* webpackChunkName: "dist/js/course-plans" */'./components/Course/CoursePlans.vue'));
...
$(document).ready(function () {
                    ...
                    UpdateObject("payment", paymentObject(price_total_stripe, methodStripe), (error, data_payment) => {
                        if (error) {
                            ...
                            if (!secret_intent_type.trim()) {
                                UpdateObjectApi("payment/intent", paymentObject(data_payment.promotionPurchasePayment.total_price, methodStripe), (error, data) => {
                                    if (data) {
                                        ...
                                        if (methodStripe === 'Sepa') {
                                            payWithSepa(result, iban, data.clientSecret, data_payment.promotionPurchase, data.customer, data.id)
                                        } else {
                                            payWithCard(result, card, data.clientSecret, data_payment.promotionPurchase, data.customer, data.id);
                                        }
                                ...
                                UpdateObjectApi(`payment/intent/${secret_intent_type}`, paymentObject(data_payment.promotionPurchasePayment.total_price, methodStripe), (error, data) => {
                                    if (data) {
    ...
    $("#submit-transfer").click(function () {
        ...
        let validate = validateForm()
        if (validate === true) {
            loading(true, "#submit-transfer");
            UpdateObject("payment", paymentObject(price_total_stripe, 'transfer'), (error, data_payment) => {
                if (error) {
                    Sentry.captureMessage(error);
                    showErrors("", false)
                    loading(false, "#submit-transfer");
                    console.log("payment", error)
                    if (error.status === 422) {
                        let dataError = error.data ? error.data : ''
                        let errorPhone = dataError.errors.phone ? dataError.errors.phone : '';
                    ...
                    console.log("payment transfer", data_payment)
                    payWithTransfer(data_payment.promotionPurchase)
                }
            })
        }
    })
    ...
    var payWithCard = function (stripe, card, clientSecret, dataUserPayment, customer, PaymentIntentId) {
        ...
        stripe
            ...
            .then(function (result) {
                ...
                if (result.error) {
                    ...
                    UpdateObjectApi("payment/update", paymentUserObject(dataUserPayment, result.error.payment_intent, customer, 'error', result.error.message), (error, data_error) => {
                    ...
                    UpdateObjectApi("payment/update", paymentUserObject(dataUserPayment, result.paymentIntent, customer, 'succeeded', null), (error, data_payment) => {
                        orderComplete();
                        if (error) {
                            console.log(error)
                            Sentry.captureMessage(error);
    ...
    let payWithSepa = function (stripe, iban, clientSecret, dataUserPayment, customer, PaymentIntentId) {
        ...
        stripe
            ...
            .then(function (result) {
                ...
                if (result.error) {
                    ...
                    UpdateObjectApi("payment/update", paymentUserObject(dataUserPayment, result.paymentIntent, customer, 'pending', null), (error, data_payment) => {
                        orderComplete();
                        if (error) {
    ...
    let paymentUserObject = function (dataUser, paymentIntent, customer, status, status_reason) {
        return {
    ...
    let payWithTransfer = function (promotionPurchase) {
        loading(true, "#submit-transfer")
        UpdateObjectApi("payment/update/" + promotionPurchase.id, paymentUserObjectTransfer('pending', 'transfer'), (error, data_payment) => {
            if (error) {
                console.log(error)
                Sentry.captureMessage(error);
            } else {
            ...
            purchaseInProcess = false;
            window.location.href = '/es/payment/' + course_id + '/' + promotionPurchase.id + '/success';
        })
    }
    let paymentUserObjectTransfer = function (status, method) {
        return {
            promotion_purchase_status: status,
            payment_method: method,
        }
    ...
    let paymentUserObjectPaypal = function (status, method,data, details) {
        let objReturn = {
            promotion_purchase_status: status,
            payment_method: method,
        }
        if (data) {
            if ('orderID' in data) {
                objReturn['orderID'] = data.orderID
            }
            if ('payerID' in data) {
                objReturn['payerID'] = data.payerID
            }
        }
        if ('purchase_units' in details) {
            if (details.purchase_units.length>0) {
                objReturn['payments_paypal'] = details.purchase_units[0].payments
            }
        }
        return objReturn;
    }
    let promotionPurchaseID
    let price
    Event.$on('price_total', (price_total) => {
        price = price_total
    })
    ...
    async function initPayPalButton() {
        paypal.Buttons({
            ...
            createOrder: async function (data, actions) {
                $("#errors").html("");
                $("#errors").hide()
                price = $('#total_price_coupon').val()
                if (!price || price === '')
                    price = price_total_stripe
                console.log("total price paypal" + price)
                let validate = validateForm()
                if (validate === true) {
                    loading(true);
                    try {
                        let data_payment = await UpdateObject("payment", paymentObject(price, 'paypal'), async (error, data_payment) => {
                            if (data_payment) {
                                console.log("payment paypal -> ok", data_payment)
                                return await data_payment;// return next()
                            } else {
                                console.log("payment paypal -> error", error)
                                Sentry.captureMessage(error);
                                showErrors("", false)
                                loading(false);
                                if (error.status === 422) {
                        ...
                        if (!data_payment.errors) {
                            promotionPurchaseID = data_payment.promotionPurchase.id
                            console.log("PromotionPurchaseID ", promotionPurchaseID)
                            if (!price || price === '')
                                price = data_payment.promotionPurchasePayment.total_price
                            console.log("with price paypal ", price)
            ...
            onApprove: function (data, actions) {
                if (promotionPurchaseID) {
                    return actions.order.capture().then(function (details) {
                        console.log(details)
                        UpdateObjectApi("payment/update/" + promotionPurchaseID, paymentUserObjectPaypal('succeeded', 'paypal',data, details), (error, data_payment) => {
                            //alert('Transaction completed by ' + details.payer.name.given_name + '!')
                            if (error) {
                                console.log(error)
                                Sentry.captureMessage(error);
                                purchaseInProcess = false;
                                window.location.href = '/es/payment/' + course_id + '/' + promotionPurchaseID + '/success'//?coupon_id='+data_payment.coupon_id;
                            } else {
                                console.log("update data payment", data_payment)
                                purchaseInProcess = false;
                                window.location.href = '/es/payment/' + course_id + '/' + data_payment.promotionPurchase.id + '/success';
            ...
            onError: function (err) {
                console.log(err);
                purchaseInProcess = false;
                // showErrors('Ha habido un error con tus credenciales', true, 'errors')
